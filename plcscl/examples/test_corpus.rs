...
fn main() {
    let corpus_dir = env::var("DATAPLC_DIR")
        .map(|p| PathBuf::from(p).join("SCL"))
        .unwrap_or_else(|_| PathBuf::from("../../../dataplc/SCL"));
        ...
        match fs::read_to_string(path) {
            Ok(content) => {
                ...
                match parse_scl_secure(&content, limits.clone()) {
                    Ok(_) => {
                        passed += 1;
                        println!("[PASS] {}", rel_path.display());
                    }
                    Err(e) => {
                        if e.to_string().contains("Security") {
                            security_blocked += 1;
                            println!("[SEC] {} - {}", rel_path.display(), e);
                        } else {
                            failed += 1;
                            println!("[FAIL] {} - {}", rel_path.display(), e);
                        }
                    }
                }
            }
            Err(e) => {
                failed += 1;
                println!("[FAIL] {} - Failed to read: {}", rel_path.display(), e);
            }
        }
    }
